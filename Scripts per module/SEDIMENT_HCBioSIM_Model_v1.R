...
d = read.csv(file="Sediment_Final_Dataset_11022021.csv")

#Trimming redundant n-alkyl fragments C3 - C18
for (i in 1:length(d$Data.ID.No)){
  if (d[i,36]==1) {d[i,28:35]=0}
  if (d[i,35]==1) {d[i,28:34]=0}
  if (d[i,34]==1) {d[i,28:33]=0}
  if (d[i,33]==1) {d[i,28:32]=0}
  if (d[i,32]==1) {d[i,28:31]=0}
  if (d[i,31]==1) {d[i,28:30]=0}
  if (d[i,30]==1) {d[i,28:29]=0}
  if (d[i,29]==1) {d[i,28]=0}
  }
...
m3 = cubist(x = train[,c(10, 11, 12, 13, 15,16, seq(17, 84,1))], y=train$log.DT50.)
P3 = predict(m3, test)
all_P3 = predict(m3, d)
train_P3 = predict(m3, train)

#Evaluating the model performance
(cor(P3, test$log.DT50.))^2
RMSE(actual = test$log.DT50., predicted = P3)

class = unique(d$Chemical.Class)
aromatics = class[grep(pattern = "A|a", x = class)]
nonarom = class[grep(pattern = "A|a", x = class, invert = TRUE)]
classv2 = c(as.character(nonarom[c(5,4,3)]), as.character(aromatics[c(1,2,3,6,4,5)]), as.character(nonarom[c(1,2)]))
arom_col = c(brewer.pal(n = (length(classv2)-2), name = "RdBu"), "darkgreen", "darkmagenta")
...
RMSE_train = RMSE(actual = train$log.DT50., train_P3)
R2_train= cor(train$log.DT50.,train_P3)^2
RMSE_test = RMSE(actual = test$log.DT50., P3)
R2_test= cor(test$log.DT50.,P3)^2
...
#mtext(side = 1, at = 10000, line = -3, text = str_c("RMSE (test) = ", signif(RMSE_test,3)), font=2, cex=1.2, xpd=NA)
#mtext(side = 1, at = 10000, line = -2, text = str_c("R-sq (test) = ", signif(R2_test,3)), font=2, cex=1.2, xpd=NA)
...
dev.copy2pdf(file="Sed_DT50_Predictions_ALL_v3.pdf")
write.csv(all_P3, file="Sed_DT50_Predictions_ALL.csv")
write.csv(test$log.DT50., file="Sed_pred_test_DT50.csv")
write.csv(P3, file="Sed_exp_test_DT50.csv")
...
#points(x=d$DT50, y=(10^(all)), pch=21, col="grey70", bg="grey70", xpd=NA, cex=1.2)
for (i in 1:length(classv2)){
  points(x=d$DT50[d$Chemical.Class==as.character(classv2[[i]])], y=4*(d$HL_bhcw[d$Chemical.Class==as.character(classv2[[i]])]), pch=21, col="black", bg=arom_col[[i]], xpd=NA, cex=1.6)
}
#points(x=d$DT50[D_miss==0], y=(10^(noD)), pch=21, col="red", bg="red", xpd=NA, cex=1.2)
#points(x=d$DT50[T_miss==0 & D_miss==0], y=(10^(noT)), pch=21, col="goldenrod", bg="goldenrod", xpd=NA, cex=1.1)
#points(x=d$DT50[T_miss==0 & D_miss==0 & OC_miss==0], y=(10^(noOC)), pch=21, col=alpha(colour = "darkgreen", alpha=0.75), bg=alpha(colour = "darkgreen", alpha=0.75), xpd=NA, cex=1.2)
...
RMSE_train_old = RMSE(actual = train$log.DT50.[train$logHL_bhcw!=-99], train$logHL_bhcw[train$logHL_bhcw!=-99]+log10(4))
R2_train_old= cor(train$log.DT50.[train$logHL_bhcw!=-99],train$logHL_bhcw[train$logHL_bhcw!=-99])^2
RMSE_test_old = RMSE(actual = test$log.DT50.[test$logHL_bhcw!=-99], (log10(4)+test$logHL_bhcw[test$logHL_bhcw!=-99]))
R2_test_old= cor(test$log.DT50.[test$logHL_bhcw!=-99],(log10(4)+test$logHL_bhcw[test$logHL_bhcw!=-99]))^2
#mtext(side = 1, at = 10000, line = -5, text = str_c("RMSE (train) = ", signif(RMSE_train,3)), font=2, cex=0.8, xpd=NA)
#mtext(side = 1, at = 10000, line = -4, text = str_c("R-sq (train) = ", signif(R2_train,3)), font=2, cex=0.8, xpd=NA)
#mtext(side = 1, at = 10000, line = -3, text = str_c("RMSE (test) = ", signif(RMSE_test,3)), font=2, cex=1.2, xpd=NA)
#mtext(side = 1, at = 10000, line = -2, text = str_c("R-sq (test) = ", signif(R2_test,3)), font=2, cex=1.2, xpd=NA)
mtext(side=1, outer=TRUE, text = "Observed log(DT50) [days]", line = 1, at = 0.5, font = 2, xpd=NA, cex=1.4)
mtext(side=2, outer=TRUE, text = "Predicted log(DT50) [days]", line = 2.5, at = 0.5, font = 2, xpd=NA, cex=1.4)
mtext(side=3, outer=TRUE, text = "BioHCWin - Sediment (4:1 IMEF)", line = 0.0, at = 0.5, font = 2, xpd=NA, cex=1.4)

dev.copy2pdf(file="BIOHCWIN_Sed_DT50_Predictions_ALL_v3.pdf")
...
boxplot((all_P3 - d$log.DT50.)~d$StudyT, ylim=c(-2,2), xlab="Test Temperate [oC]", ylab="Logarithmic Residuals [-]", las=1, xpd=NA, varwidth=TRUE)
...
boxplot((all_P3 - d$log.DT50.)~d$log_Dose, ylim=c(-2,2), xlab="LOG HC Dose [mg/kg]", ylab="Logarithmic Residuals [-]", las=1, xpd=NA, varwidth=TRUE)
abline(h=1, lty=3, lwd=1)
abline(h=-1, lty=3, lwd=1)
abline(h=0.3, lty=6, lwd=1)
abline(h=-0.3, lty=6, lwd=1)
...
boxplot((all_P3 - d$log.DT50.)~d$logWSratio, ylim=c(-2,2), xlab="LOG Water:Sediment Ratio [-]", ylab="Logarithmic Residuals [-]", las=1, xpd=NA, varwidth=TRUE)
abline(h=1, lty=3, lwd=1)
abline(h=-1, lty=3, lwd=1)
abline(h=0.3, lty=6, lwd=1)
abline(h=-0.3, lty=6, lwd=1)
...
boxplot((all_P3 - d$log.DT50.)~d$logfoc, ylim=c(-2,2), xlab="LOG foc [-]", ylab="Logarithmic Residuals [-]", las=1, xpd=NA, varwidth=TRUE)
abline(h=1, lty=3, lwd=1)
abline(h=-1, lty=3, lwd=1)
abline(h=0.3, lty=6, lwd=1)
abline(h=-0.3, lty=6, lwd=1)
...
mtext(text = "LOG Water:Sediment Ratio [-]", side=1, at=0.25, line=0, outer=TRUE, xpd=NA, font=2)
mtext(text = "log Foc [-]", side=1, at=0.75, line=0, outer=TRUE, xpd=NA, font=2)
mtext(text = "Logarithmic Residual [-]", side=2, at=0.25, line=-34, outer=TRUE, xpd=NA, font=2)
...
dev.copy2pdf(file="Sediment_System_Residuals_v1.pdf")
...
boxplot((all_P3 - d$log.DT50.)~d$Carbon.Number, ylim=c(-2,2), xlab="", ylab="", las=1, xpd=NA, varwidth=TRUE, cex.axis=0.8)
abline(h=1, lty=3, lwd=1)
abline(h=-1, lty=3, lwd=1)
abline(h=0.3, lty=6, lwd=1)
abline(h=-0.3, lty=6, lwd=1)
...
boxplot((all_P3 - d$log.DT50.)~d$Chemical.Class, ylim=c(-2,2), xlab="", ylab="", las=1, xpd=NA, varwidth=TRUE, cex.axis=0.8)
abline(h=1, lty=3, lwd=1)
abline(h=-1, lty=3, lwd=1)
abline(h=0.3, lty=6, lwd=1)
abline(h=-0.3, lty=6, lwd=1)
...
dev.copy2pdf(file="Sediment_CnClass_Residuals_v1.pdf")
